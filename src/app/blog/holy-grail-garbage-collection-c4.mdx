...
tags: [BLOG_TAGS.GARBAGE_COLLECTION, BLOG_TAGS.ALGORITHMS]
...
*   **Memory Leaks:** The program allocates memory but fails to deallocate it after it's no longer needed. Over time, available memory dwindles, leading to slowdowns and eventual crashes.
    {/* IMPLEMENTED: Simple C/C++ loop illustrating a basic memory leak */}
    <Mermaid code={`
    flowchart LR
    ...
    void memory_leak_example() {
        while (1) {
            ...
            // Each iteration loses 1KB of memory
            // Eventually the program will run out of memory
        }
    }
    
    // Another common leak pattern
    void subtle_leak_example() {
        ...
        // After loop: 256KB of memory leaked!
    }
    ```
...
*   **Dangling Pointers/References:** The program attempts to use memory that has already been deallocated. This can lead to data corruption, crashes, or security vulnerabilities as the program might be reading or writing to memory now used by another part of the system or even another program.
    {/* IMPLEMENTED: C/C++ dangling pointer example */}
    <Mermaid code={`
    flowchart LR
    ...
    void dangling_pointer_example() {
        ...
        *ptr = 42;
        ...
        // DANGER: Using the pointer after free
        *ptr = 10;  // Undefined behavior!
        ...
        // - Protected, causing a segmentation fault
    }
    ...
    void use_after_free_bug() {
        ...
        if (current->data == 1) {  // Undefined behavior!
            printf("This might work... or crash\n");
        }
    }
    ```
...
*   **Double Frees:** The program attempts to deallocate the same block of memory multiple times. This can corrupt the memory manager's internal data structures, leading to unpredictable behavior or crashes.
    {/* IMPLEMENTED: C/C++ double free example */}
    <Mermaid code={`
    flowchart LR
    ...
    void double_free_example() {
        ...
        *ptr = 100;
        ...
        // First free - this is fine
        free(ptr);
        
        // Second free - UNDEFINED BEHAVIOR!
        free(ptr);  // Double free error
        
        // This corrupts the memory allocator's internal structures
        // Can lead to crashes, security vulnerabilities, or silent corruption
    }
    
    // Common scenario leading to double free
    void conditional_double_free(int condition1, int condition2) {
        ...
        if (condition2) {
            // Developer forgot buffer might already be freed
            free(buffer);  // Double free if both conditions were true!
        }
    }
    
    // Better practice: Set pointer to NULL after free
    void safe_free_pattern() {
        ...
        free(ptr);  // This is now safe (free(NULL) is a no-op)
    }
    ```
...
*   **The "How":**
    ...
    **Reference Counting with Cycle Problem:**
    ...
    graph TB
        ...
        subgraph Cycle["Unreachable Cycle ⚠️ - Memory Leak"]
            direction LR
        ...
        Root1 ==>|points to| A
        Root2 -.->|was pointing to<br/>❌ ref removed| B
...
*   **The Problems Uncovered (Cons & Challenges):**
    ...
    *   **The Achilles' Heel - Circular References:** Simple reference counting cannot reclaim objects involved in a reference cycle. If Object A points to Object B, and Object B points back to Object A, their reference counts will each be at least 1 (due to the mutual reference). If all *external* references to this A-B pair are removed, they become unreachable garbage, but their counts remain positive, so they are never reclaimed. This leads to memory leaks, undermining one of the primary goals of automatic memory management.
    {/* IMPLEMENTED: Python example showing reference cycle formation and collection issue with basic RC */}
...
*   **The "How":** The heap is divided into two or more "generations":
    ...
    *   **Old Generation (or Tenured Space):** This area holds longer-lived objects. It's typically much larger than the Young Generation. Collections here are less frequent but more time-consuming (often called "major GCs" or "full GCs" if they involve the entire heap). These collections often use algorithms like Mark-Sweep or Mark-Compact.
    A crucial part of generational collection is managing "inter-generational pointers" – references from objects in the Old Generation to objects in the Young Generation. These must be tracked (often using "card tables" and "write barriers") so that a Young Generation collection doesn't mistakenly reclaim young objects still referenced by old objects.
...
{/* IMPLEMENTED: Sequence diagram: App Thread running & modifying objects. GC Marking Thread concurrent. Illustrate Write Barrier: App modifies pointer -> Write Barrier logs change -> GC sees change. */}
<Mermaid code={`
sequenceDiagram
...
{/* IMPLEMENTED: Sequence diagram: App Thread attempts read -> Read Barrier intercepts -> Checks forwarding -> Returns object from new/old location (healing pointer if moved). */}
<Mermaid code={`
sequenceDiagram
    participant AppThread as Application Thread
...
{/* IMPLEMENTED: Sequence/Timing diagram: App Thread A runs -> Hits Safe Point & Yields -> GC Thread runs small Work Unit -> App Thread A resumes quickly. Show multiple app threads and GC threads working in an interleaved fashion. */}
<Mermaid code={`
sequenceDiagram
...
{/* IMPLEMENTED: Timeline/flowchart: Ref Counting -> Mark-Sweep -> Mark-Compact -> Generational -> CMS -> G1 -> C4 -> ZGC/Shenandoah, highlighting C4's key contributions (e.g., concurrent compaction, barrier mechanisms) and how they flow into ZGC/Shenandoah. */}
<Mermaid code={`
graph TD
