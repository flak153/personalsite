...
{/* VISUAL_IDEA: Four small diagrams, one for each peril:
    1. Memory Leak: Show memory blocks filling up over time, with some old ones never being freed.
    2. Dangling Pointer: Show a pointer to a memory block, then the block is freed, then the pointer tries to access it (perhaps with a "Crash!" or "Corrupt Data!" annotation).
    3. Double Free: Show a memory block, it's freed, then an attempt to free it again, leading to a corrupted free list.
    4. Fragmentation: Show a memory bar with small allocated blocks and small free blocks interspersed, with a note "Cannot allocate large object even if total free space is sufficient."
*/}
